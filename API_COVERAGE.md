...
| `files[].data` | `files[].data` write-only | **not read back** | Sent when `files_revision` (string content digest of non-secret bundle identity) changes or when observable file metadata changes and `data` is still in configuration |
...
| `env` values | `env` write-only | **not read back** | Opaque `env_revision` string; do not hash secret values. `env_keys` if the API returns names. Adding/removing keys while `env` remains in configuration errors naming `env_revision` when the revision is unchanged. |
